...
void
qhg_write_mom_mesons(char fname[], qhg_correlator corr)
{
  ...
  for(int i=0; i<np; i++)
    nlines[i] = 0;
  lines lines_loc = lines_new(d[0]*nm*NGAMMAS*NFLAV);
  for(int t=0; t<d[0]; t++) {
    for(int n=0; n<nm; n++) {
      /* Momentum vector */
      int *k = mv[n];
  ...
  lines_glob = lines_sorted(lines_glob, NGAMMAS*NFLAV);
