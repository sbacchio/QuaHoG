...
qhg_write_mom_nucleons(char fname[], qhg_correlator corr_in)
{
  ...
  if(v3_rank == 0) {
    ...
    */
    ...
    double *subdat = qhg_alloc(sizeof(double)*2*ld[0]*NS*NS);
    _Complex double *c = corr.C;
    /* 
       Hierarchy is /flavor/interpolator/momentum with flavor one of ppm or pmm,
       and with interpolator one of the 2x2 combinations of chi_1, chi_2
    */
    for(int ifl=0; ifl<NFLAV; ifl++) {
      ...
      */
      ...
      for(int interp=0; interp<NCHAN; interp++) {
	/* 
	   Group /flav/interpolator
	*/
	asprintf(&group_tag, "%s", chan_tags[interp]);      
	...
	for(int im=0; im<nm; im++) {
	  ...
	  */
	  ...
	  hid_t dataset_id = H5Dcreate(group2_id, "corr_p", H5T_NATIVE_DOUBLE, filespace,
	  ...
	  */
	  ...
	  for(int t=0; t<ld[0]; t++)	    
	    memcpy(&subdat[t*NS*NS*2], &buf[NIDX(nm*t + im, ifl, interp, 0, 0)], sizeof(_Complex double)*NS*NS);
